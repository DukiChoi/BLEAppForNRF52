...
#define DEVICE_1_ADDRESS 0x28 // 첫 번째 장치의 I2C 주소
#define DEVICE_2_ADDRESS 0x29 // 두 번째 장치의 I2C 주소
...
Adafruit_BNO055 bno2 = Adafruit_BNO055(55, 0x29, &Wire); /* B Sensor Address */
...
void loop(void)
{
  ...
  // Wire.endTransmission(); // 첫 번째 장치와의 통신을 종료합니다.
  // Wire.beginTransmission(DEVICE_2_ADDRESS); // 두 번째 장치의 I2C 주소로 통신을 시작합니다.
  ...
  bno2.getEvent(&linearAccelData2, Adafruit_BNO055::VECTOR_LINEARACCEL);
  bno2.getEvent(&magnetometerData2, Adafruit_BNO055::VECTOR_MAGNETOMETER);
  bno2.getEvent(&accelerometerData2, Adafruit_BNO055::VECTOR_ACCELEROMETER);
  bno2.getEvent(&gravityData2, Adafruit_BNO055::VECTOR_GRAVITY);

  Serial.println("<BNO-1 sensor's data>");
  ...
  Serial.println("<BNO-2 sensor's data >");
  printEvent(&orientationData2);
  printEvent(&angVelocityData2);
  printEvent(&linearAccelData2);
  printEvent(&magnetometerData2);
  printEvent(&accelerometerData2);
  printEvent(&gravityData2);

  int8_t boardTemp = bno.getTemp();
  int8_t boardTemp2 = bno2.getTemp();
  Serial.println();
  Serial.print(F("temperature: "));
  Serial.print(boardTemp);
  Serial.print(F(" and "));
  Serial.println(boardTemp2);
